...
vrp_file_path = r'C:\Users\82102\Desktop\examples\data\multi_modal_data.vrp'
sol_file_path = r'C:\Users\82102\Desktop\examples\data\multi_modal_data.sol'
...
for i in range(num_iterations):
    if i == 0:
        ...
        objective_value = MultiModalState(repaired_state).objective()
        ...
        if np.exp((MultiModalState(current_states[-1]).objective() - MultiModalState(repaired_state).objective()) / temperature) >= rnd.random():
            current_states.append(repaired_state)
            objective_value = MultiModalState(repaired_state).objective()
            objectives.append(objective_value)
        else:
            ...
            objectives.append(MultiModalState(current_states[-1]).objective())
...
print("\nBest Objective Value:",MultiModalState(current_states[min_index]).objective())
print("Best Solution:",MultiModalState(current_states[min_index]).routes)
print("Iteration #:",min_index)
pct_diff = 100 * (MultiModalState(current_states[min_index]).objective() - init.objective()) / init.objective()
